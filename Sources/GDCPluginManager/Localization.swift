...
enum L {
    ...
    private static let table: [String: [AppLanguage: String]] = [
        ...
        "help.powergrade.body": [.ro: "Diferit de DCTL/LUT/Fuse: aici Resolve trebuie SĂ FIE DESCHIS, nu închis. Cu DaVinci Resolve Studio deschis, „Instalează” creează automat un album propriu în Gallery pentru fiecare produs (numit „GDC PowerGrades — <nume produs>”), fără să amestece produsele între ele — dacă un produs are mai multe .drx, intră toate în același album. Pe Resolve gratuit, cu Resolve închis sau dacă automatul eșuează, fișierele tot se descarcă și se verifică — le imporți manual din Gallery într-un pas (click-dreapta pe album → Import).", .en: "Different from DCTL/LUT/Fuse: here Resolve needs to be OPEN, not closed. With DaVinci Resolve Studio open, “Install” automatically creates its own Gallery album per product (named “GDC PowerGrades — <product name>”), so products never mix — if one product has several .drx files, they all land in that same album. On the free edition, with Resolve closed, or if the automatic step fails, the files still download and get verified — you import them manually from the Gallery in one step (right-click the album → Import).", .es: "A diferencia de DCTL/LUT/Fuse: aquí Resolve debe estar ABIERTO, no cerrado. Con DaVinci Resolve Studio abierto, “Instalar” crea automáticamente un álbum propio en la Gallery para cada producto (llamado “GDC PowerGrades — <nombre del producto>”), sin mezclar productos — si un producto tiene varios .drx, todos entran en el mismo álbum. En la edición gratuita, con Resolve cerrado, o si el paso automático falla, los archivos igual se descargan y se verifican — los importas manualmente desde la Gallery en un paso (clic derecho en el álbum → Importar)."],
