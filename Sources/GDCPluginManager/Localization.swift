...
enum L {
    ...
    private static let table: [String: [AppLanguage: String]] = [
        ...
        "powergrade.imported": [.ro: "Adăugat automat în Gallery, albumul „GDC PowerGrades”.", .en: "Added automatically to the Gallery, in the \"GDC PowerGrades\" album.", .es: "Añadido automáticamente a la Gallery, en el álbum \"GDC PowerGrades\"."],
        "powergrade.manualstep": [.ro: "Fișierele sunt verificate în %@ — deschide Gallery-ul din Resolve și importă-le manual (click-dreapta pe albumul „GDC PowerGrades” → Import).", .en: "The files are verified in %@ — open Resolve's Gallery and import them manually (right-click the \"GDC PowerGrades\" album → Import).", .es: "Los archivos están verificados en %@ — abre la Gallery de Resolve e impórtalos manualmente (clic derecho en el álbum \"GDC PowerGrades\" → Importar)."],
        "powergrade.manualremove": [.ro: "Fișierele locale au fost șterse — elimină-l și din Gallery manual (Resolve închis sau scripting indisponibil).", .en: "The local files were removed — remove it from the Gallery manually too (Resolve was closed or scripting wasn't available).", .es: "Los archivos locales se eliminaron — elimínalo también de la Gallery manualmente (Resolve estaba cerrado o el scripting no estaba disponible)."],
        ...
        "help.powergrade.body": [.ro: "Diferit de DCTL/LUT/Fuse: aici Resolve trebuie SĂ FIE DESCHIS, nu închis. Cu DaVinci Resolve Studio deschis, „Instalează” îl adaugă automat în Gallery, în albumul „GDC PowerGrades”. Pe Resolve gratuit, cu Resolve închis sau dacă automatul eșuează, fișierele tot se descarcă și se verifică — le imporți manual din Gallery într-un pas (click-dreapta pe album → Import).", .en: "Different from DCTL/LUT/Fuse: here Resolve needs to be OPEN, not closed. With DaVinci Resolve Studio open, “Install” adds it automatically to the Gallery, in the “GDC PowerGrades” album. On the free edition, with Resolve closed, or if the automatic step fails, the files still download and get verified — you import them manually from the Gallery in one step (right-click the album → Import).", .es: "A diferencia de DCTL/LUT/Fuse: aquí Resolve debe estar ABIERTO, no cerrado. Con DaVinci Resolve Studio abierto, “Instalar” lo añade automáticamente a la Gallery, en el álbum “GDC PowerGrades”. En la edición gratuita, con Resolve cerrado, o si el paso automático falla, los archivos igual se descargan y se verifican — los importas manualmente desde la Gallery en un paso (clic derecho en el álbum → Importar)."],
