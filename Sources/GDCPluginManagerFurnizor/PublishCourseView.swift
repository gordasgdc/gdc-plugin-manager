...
struct PublishCourseView: View {
    ...
    private func applyWorkspaceSelection(_ id: String?) {
        ...
        if let entry = existingCourses.first(where: { $0.id == id }) { load(entry) }
    }
    ...
    private var editorBody: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 18) {
                ...
                if embedded, let selectedID = editingID, let entry = existingCourses.first(where: { $0.id == selectedID }) {

                    Button("Șterge…", role: .destructive) { pendingDelete = entry }

                }

                if !embedded && !existingCourses.isEmpty {
                    Divider()
