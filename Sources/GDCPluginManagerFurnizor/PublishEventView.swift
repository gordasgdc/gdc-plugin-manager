...
struct PublishEventView: View {
    ...
    private func applyWorkspaceSelection(_ id: String?) {
        ...
        if let entry = existingEvents.first(where: { $0.id == id }) { load(entry) }
    }
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 18) {
                ...
                HStack {
                    ...
                    Button(editingID == nil ? "Publică" : "Actualizează") { publishGate.request(title) { await publish() } }
                ...
                if embedded, let selectedID = editingID, let entry = existingEvents.first(where: { $0.id == selectedID }) {

                    Button("Șterge…", role: .destructive) { pendingDelete = entry }

                }

                if !embedded && !existingEvents.isEmpty {
                    Divider()
