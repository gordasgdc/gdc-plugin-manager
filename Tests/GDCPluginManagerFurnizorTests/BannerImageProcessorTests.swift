...
final class BannerImageProcessorTests: XCTestCase {
    ...
    func testWrongAspectTooSmallAndTooHeavyAreRejected() throws {
        ...
        XCTAssertThrowsError(try BannerImageProcessor.process(try png(2400, 400, noise: true), slot: .standard)) {
            guard case BannerImageProcessor.Failure.tooLarge = $0 else { return XCTFail("\($0)") }
        }
